"""
NYC Music Organizations — from the City Planning Facilities DB.

Source: https://data.cityofnewyork.us/City-Government/Facilities-Database/ji82-xba5
DCLA-funded music organizations including orchestras, choruses, jazz
ensembles, and youth music programs across all five boroughs.
"""
...
class MusicSource(DataSource):
    ...
    def description(self) -> str:
        return "NYC Music Organizations & Ensembles"
    ...
    def fetch(self) -> list[Venue]:
        ...
        type_list = ",".join(f"'{t}'" for t in _MUSIC_TYPES)
        params: dict[str, Any] = {
            "$limit": 5000,
            "$where": (
        ...
        log.info("Music: fetching Facilities Database …")
        resp = requests.get(FAC_BASE_URL, params=params, timeout=30)
        resp.raise_for_status()
        rows = resp.json()
        log.info("Music: %d rows from Facilities DB", len(rows))
        ...
        for row in rows:
            ...
            if v:
                ...
                bad += 1
        log.info("Music: %d venues (from %d rows, %d bad coords)", len(venues), len(rows), bad)
        return venues
