...
# Cache paths are set dynamically via self.cache_dir (from DataSource base class)
...
class GrocerySource(DataSource):
    ...
    @property
    def url(self) -> str:
        return "https://data.ny.gov/Economic-Development/Retail-Food-Stores/9a8c-vfzj"
    ...
    def _load_snap_keys(self) -> set[str]:
        ...
        snap_cache = self.cache_dir / "snap_retailers.zip"
        self.cache_dir.mkdir(parents=True, exist_ok=True)

        # Download zip if not cached (30-day TTL; data is annual)
        if snap_cache.exists():
            import time
            age_h = (time.time() - snap_cache.stat().st_mtime) / 3600
            if age_h > 30 * 24:
                snap_cache.unlink()
        ...
        if not snap_cache.exists():
            ...
            try:
                ...
                Path(tmp_path).rename(snap_cache)
                log.info("SNAP enrichment: downloaded %.1f MB",
                         snap_cache.stat().st_size / 1e6)
        ...
        try:
            with zipfile.ZipFile(snap_cache) as zf:
