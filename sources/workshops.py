"""
NYC Libraries, Workshops & Maker-Adjacent Spaces — from the City Planning Facilities DB.

Source: https://data.cityofnewyork.us/City-Government/Facilities-Database/ji82-xba5
Public libraries (many with maker programs, 3D printers, laser cutters),
craft/design cultural organizations, and new media groups.
"""
...
# Libraries + creative/maker-adjacent cultural types
_MAKE_TYPES = (
    "PUBLIC LIBRARY",
    "CRAFTS",
    "FOLK ARTS",
...
class WorkshopsSource(DataSource):
    @property
    def name(self) -> str:
        return "workshops"
    ...
    def description(self) -> str:
        return "NYC Libraries, Workshops & Maker Spaces"
    ...
    def fetch(self) -> list[Venue]:
        ...
        type_list = ",".join(f"'{t}'" for t in _MAKE_TYPES)
        params: dict[str, Any] = {
            "$limit": 5000,
            "$where": (
        ...
        log.info("Make: fetching Facilities Database …")
        resp = requests.get(FAC_BASE_URL, params=params, timeout=30)
        resp.raise_for_status()
        rows = resp.json()
        log.info("Make: %d rows from Facilities DB", len(rows))
        ...
        for row in rows:
            ...
            if v:
                ...
                bad += 1
        log.info("Make: %d venues (from %d rows, %d bad coords)", len(venues), len(rows), bad)
        return venues
    ...
    def _venue_from_row(row: dict[str, Any]) -> Venue | None:
        ...
        tags = [_TYPE_TAGS.get(factype, "workshop")]
