...
Maintains a persistent SQLite cache (data/crossref.db at repo root) so the
process can be resumed across daily runs.  Each run processes up to the daily
API limit, then stops.  The next run picks up where it left off.
